...
export default function ChatTab({ projectId }) {
  ...
  return (
    <div className="chat-root relative min-h-screen bg-gray-950 text-gray-100 -m-6 p-6 overflow-hidden">
        <div className="relative z-10 flex h-[calc(100vh-4rem)]">
          <div className="w-72 flex flex-col">
            <div className="p-4 border-b border-white/[0.07]">
              <button
                onClick={() => setShowModeSelector(true)}
                className="w-full glow-sky px-4 py-2.5 text-sm font-semibold rounded-xl transition-all hover:-translate-y-px flex items-center justify-center gap-1.5"
              >
              ...
              {projectSessions.length === 0 ? (
                ...
                projectSessions.map((session) => (
                  <div
                    ...
                    className={`session-card w-full p-3 rounded-xl text-left cursor-pointer ${
                      activeSession?.id === session.id ? 'active' : ''
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-gray-100 truncate text-sm">
          ...
          <div className="flex-1 flex flex-col ml-6">
            {showModeSelector && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-8">
                <div className="bg-gray-900/95 rounded-2xl p-6 max-w-lg w-full border border-white/10 fade-up">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold text-gray-100">Choose Chat Mode</h2>
                    <button
                    ...
                    {CHAT_MODES.map((mode) => (
                      ...
                      >
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className="font-semibold text-gray-100">{mode.name}</span>
                        </div>
                  ...
                  <button
                    onClick={() => setShowModeSelector(false)}
                    className="w-full mt-4 ghost-btn px-4 py-2.5 text-sm font-medium text-gray-400 rounded-xl"
                  >
                    Cancel
                  </button>
                </div>
                      ...
                      >
                        ...
                        >
                          ...
                          {msg.injected_thought && (
                            <div className="ai-insight mt-3 p-3 rounded-lg">
                ...
                <div className="p-4 border-t border-white/[0.07]">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="glow-sky px-5 text-sm font-semibold rounded-xl transition-all hover:-translate-y-px disabled:opacity-40 flex items-center gap-1.5"
                    >
                      <FiSend size={14} /> Send
                    </button>
                  </div>
