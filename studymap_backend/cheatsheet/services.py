...
from prompts import CHEATSHEET_PROMPT
...
class CheatsheetService:
    ...
    def generate_cheatsheet(self, project_id):
        context = self.build_context(project_id)
        if not context:
            return {'error': 'Please upload documents before generating cheatsheet'}

        user_prompt = f"Context:\n{context}\n\nCreate a cheatsheet from this material:"

        response = self.call_api(CHEATSHEET_PROMPT, user_prompt)
        ...
        try:
            ...
            raise Exception("Failed to parse cheatsheet response")
